...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    // --- Dynamic performance tuning (lower cost on high DPI / mobile) ---
    ...
    if (isMobile) {
      ...
      config.CURL = Math.min(config.CURL, 2)
    } else if (vw >= 1600) {
      ...
      config.DYE_RESOLUTION = Math.min(config.DYE_RESOLUTION, 768)
      config.SIM_RESOLUTION = Math.min(config.SIM_RESOLUTION, 80)
      config.PRESSURE_ITERATIONS = Math.min(config.PRESSURE_ITERATIONS, 12)
    } else {
      config.DYE_RESOLUTION = Math.min(config.DYE_RESOLUTION, 640)
      config.SIM_RESOLUTION = Math.min(config.SIM_RESOLUTION, 64)
      config.PRESSURE_ITERATIONS = Math.min(config.PRESSURE_ITERATIONS, 10)
    }
